...
object JdbcUtils extends Logging {
  ...
  def insertStatement(table: String,
  ...
  : String = {
    ...
    val columns =  tableSchema.isEmpty match {
      case true =>
        //c1 c2 c3
        //c1,c2,c3
      ...
      case false => {
        ...
        rddSchema.fields.map(col => {
          val normalizeName = tableColumnNames.find(
            //caseInsensitiveResolution就是比较传入的而两个参数费否相等
            f => org.apache.spark.sql.catalyst.analysis.caseInsensitiveResolution(f, col.name)
          ).getOrElse {
            throw new AnalysisException(s"${col.name}不存在！")
          }
          dialect.quoteIdentifier(normalizeName)
        }).mkString(",")
      }
    }
    ...
    mode match {
      ...
      case CustomSaveMode.Ignore =>
        s"INSERT IGNOR INTO $table ($columns) VALUES ($placeholders)"
      case _ => {
        throw new IllegalArgumentException(s"$mode is illegal")
      }
    }
  ...
  def savePartition(
    ...
    try {
      ...
      val stmt = conn.prepareStatement(sqlStm)
  ...
  def getSchemaOption(conn: Connection, options: JDBCOptions): Option[StructType] = {
    ...
    try {
      ...
      //比如说一张表中有2亿条数据，获取方式select * from tb where 1=0
