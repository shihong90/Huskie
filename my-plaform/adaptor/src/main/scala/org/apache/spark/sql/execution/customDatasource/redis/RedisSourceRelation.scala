...
import org.apache.spark.sql.execution.customDatasource.redis.RedisSourceRelation._
import org.apache.spark.sql.execution.customDatasource.redis.RedisOptions._
  ...
  private implicit val redisConfig: RedisConfig = {
    ...
    val config = new RedisConfig(
      if ((parameters.keySet & Set("host", "port", "auth", "dbNum", "timeout")).isEmpty) {
        ...
        val dbNum = parameters.get("db").map(_.toInt).getOrElse(Protocol.DEFAULT_DATABASE)
