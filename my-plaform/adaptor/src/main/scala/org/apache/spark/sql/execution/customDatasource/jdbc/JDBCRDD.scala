...
import org.apache.spark.sql.execution.customDatasource.jdbc.JDBCOptions
...
object JDBCRDD extends Logging {
  ...
  def resolveTable(options: JDBCOptions): StructType = {
    //jdbc的url地址：（jdbc:mysql://cdh1:3306/数据库名?characterEncoding=utf8）
    val url = options.url
    //要查询的表名
    val table = options.table
    //获取数据库的方言（比如MySQLDialect）
  ...
  def scanTable(
    ...
    new JDBCRDD(
      ...
      pruneSchema(schema, requiredColumns),//根据requiredColumns去schema拿到对应的StructType
      quotedColumns,//需要获取的列
      filters,//过滤条件
      parts,//分区信息
      url,//jdbc的url地址
      options)
