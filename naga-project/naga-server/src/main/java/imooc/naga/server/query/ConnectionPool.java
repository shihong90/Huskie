...
public class ConnectionPool {
    ...
    private static LoadingCache<String, Connection> cache = null;//连接缓存池

    @PostConstruct//实现一个初始化的方法
    public void init() {
        cache = CacheBuilder.newBuilder()
                ...
                .build(new CacheLoader<String, Connection>() {
                    ...
                    public Connection load(String key) throws Exception {
                        ...
                        Class.forName(EngineType.valueOf(connInfo.get(1)).getDriver());//引入我们dirver信息
    ...
    public static Connection getConnection(String key) throws ExecutionException {
        // key     username|engineType  key是以竖线进行分割的字符串

        return cache.get(key);//获取到相关的连接
    }
}
